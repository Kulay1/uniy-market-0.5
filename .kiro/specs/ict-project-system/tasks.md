...
- [ ] 20. UI/UX Enhancement Sprint / UI/UX 功能增强冲刺 **NEW - Demo Features**

  > 本批次任务专注于演示所需的前端交互增强，严禁修改任何现有 CSS、全局字体、配色方案或组件间距。所有新组件必须复用现有 UI 组件库（shadcn/ui）。
  > This sprint focuses on demo-required frontend enhancements. No existing CSS, fonts, colors, or spacing may be modified. All new components must reuse the existing shadcn/ui library.

  - [ ] 20.1 Notification Bell - Message Preview Popover / 通知铃铛 - 消息预览浮窗
    - 将 Header.tsx 中铃铛 Badge 的硬编码数字从 "3" 改为 "1" / Change hardcoded badge number from "3" to "1" in Header.tsx
    - 将铃铛区域包裹在 shadcn Popover 组件中（复用现有 ui/popover.tsx）/ Wrap bell area in shadcn Popover (reuse existing ui/popover.tsx)
    - 浮窗内渲染一个示例消息条目：含头像、卖家名 "Example Seller"、消息预览文字 / Render one example message entry in popover: avatar, seller name "Example Seller", message preview text
    - 点击该条目使用 useNavigate 跳转至 /chat/example-seller / On click, navigate to /chat/example-seller using useNavigate
    - _Requirements: 导航栏交互优化_

  - [ ] 20.2 My Page - New Route and Page Component / My Page - 新路由和页面组件
    - 在 routes.ts 中新增路由 /my-page 指向新建的 MyPage.tsx / Add route /my-page in routes.ts pointing to new MyPage.tsx
    - 将 Header.tsx 中 "My Page" 导航链接改为使用 useNavigate 跳转至 /my-page / Update "My Page" nav link in Header.tsx to navigate to /my-page
    - _Requirements: 用户个人中心_

  - [ ] 20.3 My Page - Social Style Profile Header / My Page - 社交风格个人信息头部
    - 在 MyPage.tsx 顶部创建个人信息头部区域 / Create profile header section at top of MyPage.tsx
    - 左侧圆形头像（复用 Avatar 组件）/ Left side circular avatar (reuse Avatar component)
    - 右侧上方用户名（硬编码示例 "Demo User"）/ Right side username above (hardcoded "Demo User")
    - 右侧下方紧贴邮箱（硬编码示例 "demo@university.edu"）/ Right side email below (hardcoded "demo@university.edu")
    - _Requirements: 用户个人中心 - 个人信息头部_

  - [ ] 20.4 My Page - Two Full-Width Tabs / My Page - 两个全宽切换选项卡
    - 使用现有 ui/tabs.tsx 创建两个全宽选项卡：[Chat History] 和 [My Products] / Use existing ui/tabs.tsx to create two full-width tabs: [Chat History] and [My Products]
    - [Chat History] 为默认激活选项卡 / [Chat History] is the default active tab
    - _Requirements: 用户个人中心 - 选项卡_

  - [ ] 20.5 My Page - Chat History Tab with Example Entry / My Page - Chat History 选项卡示例条目
    - 在 [Chat History] 选项卡内渲染一个示例对话条目 / Render one example chat entry in [Chat History] tab
    - 条目包含：头像、卖家名 "Example Seller"、消息预览文字 / Entry contains: avatar, seller name "Example Seller", message preview text
    - 点击条目使用 useNavigate 跳转至 /chat/example-seller（与铃铛浮窗跳转目标完全一致）/ On click, navigate to /chat/example-seller (same target as bell popover)
    - _Requirements: 用户个人中心 - Chat History 选项卡_

  - [ ] 20.6 My Page - My Products Tab with Example Entry / My Page - My Products 选项卡示例条目
    - 在 [My Products] 选项卡内渲染一个示例上架产品条目 / Render one example product entry in [My Products] tab
    - 条目包含：产品图片（使用 mockProducts[0].image）、产品名称、价格、状态 Badge（"Active"）/ Entry contains: product image (use mockProducts[0].image), product name, price, status Badge ("Active")
    - _Requirements: 用户个人中心 - My Products 选项卡_

  - [ ] 20.7 Unified Chat Route - Dynamic Seller Name / 统一聊天路由 - 动态卖家名称
    - 确认 ChatPage.tsx 已通过 useParams 读取 sellerId / Confirm ChatPage.tsx reads sellerId via useParams
    - 当 sellerId 为 "example-seller" 时，顶部显示卖家名 "Example Seller" / When sellerId is "example-seller", display seller name "Example Seller" in header
    - 确保从铃铛浮窗、My Page Chat History、商品详情页三个入口跳转后，均渲染同一个 ChatPage 组件 / Ensure all three entry points (bell popover, My Page chat history, product detail) render the same ChatPage component
    - _Requirements: 统一聊天架构_

  - [ ] 20.8 Chat Bubble - Translation Toggle UI / 聊天气泡 - 翻译浮层 UI
    - 在 ChatPage.tsx 的消息气泡上添加 onClick 事件 / Add onClick event to message bubbles in ChatPage.tsx
    - 点击气泡后，在点击位置附近渲染一个小浮层，包含 "Translate" 按钮（纯 UI，不挂载 API）/ On bubble click, render a small popover near click position with a "Translate" button (UI only, no API)
    - 使用 useState 管理当前选中的消息 ID，确保同时只显示一个浮层 / Use useState to track selected message ID, ensure only one popover shows at a time
    - 点击浮层外部或再次点击气泡时关闭浮层 / Close popover when clicking outside or clicking bubble again
    - _Requirements: 即时通讯交互细节 - 翻译浮层_

  - [ ] 20.9 Auth Flow - Email Verification Page / 认证流程 - 邮箱验证页面
    - 新建页面 EmailVerificationPage.tsx / Create new page EmailVerificationPage.tsx
    - 页面显示英文提示："We have sent a code to your email. Please enter the verification code." / Page shows English prompt: "We have sent a code to your email. Please enter the verification code."
    - 使用现有 ui/input-otp.tsx 或普通 Input 渲染验证码输入框 / Use existing ui/input-otp.tsx or plain Input for verification code input
    - 输入任意 6 位数字后点击 "Verify" 按钮，模拟验证通过，跳转至主页 "/" / After entering any 6 digits and clicking "Verify", simulate success and navigate to "/"
    - 在 routes.ts 中新增路由 /verify-email / Add route /verify-email in routes.ts
    - _Requirements: 登录与注册流程增强 - 邮箱验证页_

  - [ ] 20.10 Auth Flow - Register Button Redirect to Verification / 认证流程 - 注册按钮跳转验证页
    - 修改 LoginPage.tsx 中 Register 选项卡的 handleRegister 函数 / Modify handleRegister function in Register tab of LoginPage.tsx
    - 点击 "Create Account" 按钮后，不直接跳转主页，而是跳转至 /verify-email / After clicking "Create Account", navigate to /verify-email instead of "/"
    - 保持现有的 isLoading 状态和加载动画逻辑不变 / Keep existing isLoading state and loading animation logic unchanged
    - _Requirements: 登录与注册流程增强 - 注册跳转_
